...
{
	var int_Input = parseFloat(userInput);				// changes from string to integer.
    console.log('feedback switch entered');
    												    // switch statement evaluates a case as true if the conditional has been met.
    switch(true)
    {
    												    // case 1: If the user's guess and random number are correct execute winnerFeedback( ).
    	case (ran_num == int_Input):
    		winnerFeedback();
    		break;
                                                        // case 2 - 7: The random number is subtracted by the users guess and then the absolute value is taken.
                                                        //             The number is then evaluated against the inequality to find where the value sets on the number line 1 - 100.
        case (Math.abs(ran_num - int_Input) < 5):
            console.log("Scortching");
            userFeedback = 'Scortching!';
            break;
        case (Math.abs(ran_num - int_Input) < 10):
            console.log("Hot");
            userFeedback = 'Hot!';
            break;
        case (Math.abs(ran_num - int_Input) < 25):
            console.log("Warm");
            userFeedback = 'Warm.';
            break;
        case (Math.abs(ran_num - int_Input) < 50):
            console.log("Lukewarm");
            userFeedback = 'Lukewarm.';
            break;
        case (Math.abs(ran_num - int_Input) < 80):
            console.log("Cold");
            userFeedback = 'Cold.';
            break;
        case (Math.abs(ran_num - int_Input) < 100):
